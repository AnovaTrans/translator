...
class EmbeddingMatcher:
    ...
    def format_per_segment_references(
        self,
        matches_dict: Dict[int, List[ReferenceMatch]],
        segment_ids: List[str],
        max_chars_total: int = 2000,
    ) -> Dict[str, str]:
        """
        Format per-segment reference strings from find_similar_batch output.

        Args:
            matches_dict:    {text_index: [ReferenceMatch, ...]} from find_similar_batch
            segment_ids:     segment IDs in the same order as the source_texts list
            max_chars_total: total char budget divided equally across segments

        Returns:
            {seg_id: formatted_reference_string} — only for segments with matches
        """
        if not matches_dict or not segment_ids:
            return {}

        MIN_CHARS_PER_SEG = 300
        per_seg_chars = max(
            MIN_CHARS_PER_SEG,
            max_chars_total // len(segment_ids),
        )

        result: Dict[str, str] = {}
        for idx, seg_id in enumerate(segment_ids):
            matches = matches_dict.get(idx, [])
            if matches:
                formatted = self.format_reference_context(matches, max_chars=per_seg_chars)
                if formatted:
                    result[seg_id] = formatted
        return result
